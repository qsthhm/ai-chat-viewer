...
export async function GET(
  ...
  const chats = col.chatIds
    .map(id => getChatById(id))
    .filter(Boolean)
    .map(c => ({ id: c!.id, title: c!.title, source: c!.source, createdAt: c!.createdAt }));
...
export async function PATCH(
  ...
  try {
    ...
    const col = getCollectionById(params.id);
    if (!col) return NextResponse.json({ error: '集不存在' }, { status: 404 });
    if (col.userId !== user.userId && user.role !== 'admin') {

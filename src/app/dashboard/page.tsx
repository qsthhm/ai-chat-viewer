...
interface MyCol { id:string; name:string; description:string; chatIds:string[]; shareId:string; plazaStatus:string; rejectReason:string; createdAt:string; }
const stLabel: Record<string,string> = { none:'私有', pending:'上广场请求审核中', approved:'已上广场', rejected:'未通过' };
const stColor: Record<string,string> = { none:'text-surface-400', pending:'text-amber-600', approved:'text-green-600', rejected:'text-red-500' };
...
export default function DashboardPage() {
  ...
  const [shareColModal, setShareColModal] = useState(false);
  const [shareColId, setShareColId] = useState('');
  const [shareColPasscode, setShareColPasscode] = useState('');
  const [shareColPlaza, setShareColPlaza] = useState(false);
  const [shareColUrl, setShareColUrl] = useState('');
  const [shareColSaving, setShareColSaving] = useState(false);
  // Profile
  ...
  const createCol = async () => { if(!colName.trim())return; setSaving(true); await fetch('/api/collections/create',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:colName,description:colDesc,chatIds:colChats,isPublic:false})}); toast('集已创建'); setColModal(false); setColName(''); setColDesc(''); setColChats([]); setSaving(false); loadCols(); };
  const deleteCol = async (id: string) => { if(!confirm('确定删除？'))return; await fetch(`/api/collections/${id}`,{method:'DELETE'}); toast('已删除'); loadCols(); };
  const reapply = async (chatId?: string, collectionId?: string) => {
    const res = await fetch('/api/chats/reapply',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({chatId,collectionId})});
    if(res.ok){ toast('已重新申请'); loadChats(); loadCols(); } else { const d = await res.json(); toast(d.error||'失败'); }
  };

  // Share collection modal
  const openShareCol = (col: MyCol) => { setShareColId(col.id); setShareColPasscode(''); setShareColPlaza(false); setShareColUrl(''); setShareColModal(true); };
  const confirmShareCol = async () => {
    setShareColSaving(true);
    const res = await fetch('/api/collections/share',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({collectionId:shareColId,publishToPlaza:shareColPlaza,passcode:shareColPasscode||''})});
  ...
  return (
    <div className="min-h-dvh"><Navbar/>
                ...
                <div key={c.id} className="p-4 rounded-xl border border-surface-200 bg-white hover:border-brand-200 transition-colors group">
                  <div className="flex items-center gap-3">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-3 mt-1 text-xs text-surface-400"><span>{new Date(c.createdAt).toLocaleDateString('zh-CN')}</span><span className={stColor[c.plazaStatus]||''}>{stLabel[c.plazaStatus]||''}</span></div>
                    </div>
                  ...
                  {c.plazaStatus==='rejected' && (<div className="mt-2 p-2.5 rounded-lg bg-red-50 border border-red-100"><p className="text-xs text-red-600">拒绝原因：{c.rejectReason || '未说明'}</p><button onClick={()=>reapply(c.id)} className="mt-1.5 text-xs text-brand-500 font-semibold hover:underline">重新申请上广场</button></div>)}
                </div>
              ))}</div>
            )}
          </div>
            ...
            {cols.length===0 ? <div className="text-center py-16 text-surface-400 text-sm">还没有集</div> : (
              <div className="space-y-2">{cols.map(c=>(
                <div key={c.id} className="p-4 rounded-xl border border-surface-200 bg-white hover:border-brand-200 transition-colors group">
                  ...
                  {c.plazaStatus==='rejected' && (<div className="mt-2 p-2.5 rounded-lg bg-red-50 border border-red-100"><p className="text-xs text-red-600">拒绝原因：{c.rejectReason || '未说明'}</p><button onClick={()=>reapply(undefined,c.id)} className="mt-1.5 text-xs text-brand-500 font-semibold hover:underline">重新申请上广场</button></div>)}
                </div>
              ))}</div>
            )}
          </div>
      ...
      {/* Share collection modal */}
      <Modal open={shareColModal} onClose={()=>{setShareColModal(false);setShareColUrl('');}}>
        {!shareColUrl ? (
          <div className="p-6">
            <div className="text-center mb-5"><h3 className="font-serif font-semibold text-lg">分享集</h3><p className="text-sm text-surface-500 mt-1">生成可分享的链接</p></div>
            <div className="space-y-3 mb-4">
              <div><label className="block text-xs font-medium text-surface-600 mb-1">口令保护（可选）</label><input value={shareColPasscode} onChange={e=>setShareColPasscode(e.target.value.replace(/[^a-zA-Z0-9]/g,'').slice(0,4))} placeholder="4位字母或数字" maxLength={4} className="w-full px-3 py-2 rounded-xl border border-surface-200 focus:border-brand-400 outline-none text-sm font-mono tracking-widest"/></div>
            </div>
            <label className="flex items-center gap-3 p-3 rounded-xl border border-surface-200 hover:border-brand-300 cursor-pointer transition-colors mb-4">
              <input type="checkbox" checked={shareColPlaza} onChange={e=>setShareColPlaza(e.target.checked)} className="w-4 h-4 accent-brand-500"/>
              <div><p className="text-sm font-medium">发布到广场</p><p className="text-xs text-surface-400 mt-0.5">审核通过后出现在公共广场</p></div>
            </label>
            <div className="flex gap-3">
              <button onClick={()=>{setShareColModal(false);}} className="flex-1 py-2.5 rounded-xl border border-surface-200 text-surface-600 text-sm font-semibold hover:bg-surface-50">取消</button>
              <button onClick={confirmShareCol} disabled={shareColSaving} className="flex-1 py-2.5 rounded-xl bg-brand-500 text-white text-sm font-semibold hover:bg-brand-600 disabled:opacity-50 shadow-sm">{shareColSaving?'处理中...':'确认分享'}</button>
            </div>
          </div>
        ):(
          <div className="p-6 text-center">
            <p className="text-brand-500 font-semibold text-sm mb-1">✓ 分享成功</p>
            {shareColPlaza && <p className="text-xs text-surface-400 mb-3">广场发布申请已提交</p>}
            {shareColPasscode && <p className="text-xs text-surface-500 mb-3">口令：<span className="font-mono font-semibold text-surface-800">{shareColPasscode}</span></p>}
            <div onClick={()=>{navigator.clipboard.writeText(shareColUrl);toast('链接已复制');}} className="p-3 rounded-xl border border-surface-200 hover:border-brand-300 cursor-pointer bg-surface-50"><code className="text-xs text-surface-600 break-all font-mono">{shareColUrl}</code><p className="text-xs text-brand-500 font-medium mt-2">点击复制链接</p></div>
            <button onClick={()=>{setShareColModal(false);setShareColUrl('');}} className="mt-4 text-sm text-surface-400 hover:text-brand-500">关闭</button>
          </div>
        )}
      </Modal>
    </div>

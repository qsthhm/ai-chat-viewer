...
export default function DashboardPage() {
  ...
  const saveProfile = async () => {
    ...
    toast('资料已更新'); setPCurPass(''); setPNewPass(''); setPSaving(false);
  ...
  return (
    <div className="min-h-dvh"><Navbar/>
              ...
              <div className="space-y-2">{chats.map(c=>(
                <div key={c.id} className="flex items-center gap-3 p-4 rounded-xl border border-surface-200 bg-white hover:border-brand-200 transition-colors group">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-1 text-xs text-surface-400"><span>{new Date(c.createdAt).toLocaleDateString('zh-CN')}</span><span className={statusColor[c.plazaStatus]||''}>{statusLabel[c.plazaStatus]||''}</span></div>
                  </div>
          ...
          <div className="max-w-md">
            <div className="space-y-4">
              <div>
                ...
                <input value={pNickname} onChange={e=>setPNickname(e.target.value)} className="w-full px-4 py-2.5 rounded-xl border border-surface-200 focus:border-brand-400 outline-none text-sm"/>
                <p className="text-[0.68rem] text-surface-400 mt-1">2-16字符，中文/字母/数字/下划线/短横线</p>
              </div>
              <div>
                <label className="block text-xs font-medium text-surface-600 mb-1.5">邮箱</label>
                <input type="email" value={pEmail} onChange={e=>setPEmail(e.target.value)} className="w-full px-4 py-2.5 rounded-xl border border-surface-200 focus:border-brand-400 outline-none text-sm"/>
              </div>
              <div className="border-t border-surface-100 pt-4">
                <p className="text-xs font-medium text-surface-600 mb-3">修改密码（不改可留空）</p>
                <div className="space-y-3">
                  <input type="password" value={pCurPass} onChange={e=>setPCurPass(e.target.value)} placeholder="当前密码" className="w-full px-4 py-2.5 rounded-xl border border-surface-200 focus:border-brand-400 outline-none text-sm"/>
                  <input type="password" value={pNewPass} onChange={e=>setPNewPass(e.target.value)} placeholder="新密码（至少6位）" className="w-full px-4 py-2.5 rounded-xl border border-surface-200 focus:border-brand-400 outline-none text-sm"/>
                </div>
              </div>
      ...
      <Modal open={editModal} onClose={()=>setEditModal(false)} maxWidth="max-w-sm">
        <div className="p-6"><h3 className="font-serif font-semibold text-lg mb-4">编辑对话</h3>
          ...
          <div className="flex gap-3"><button onClick={()=>setEditModal(false)} className="flex-1 py-2.5 rounded-xl border border-surface-200 text-sm font-semibold text-surface-600">取消</button><button onClick={saveEdit} className="flex-1 py-2.5 rounded-xl bg-brand-500 text-white text-sm font-semibold hover:bg-brand-600">保存</button></div>
        </div>
      </Modal>
      <Modal open={colModal} onClose={()=>setColModal(false)}>
        <div className="p-6"><h3 className="font-serif font-semibold text-lg mb-4">新建集</h3>
          <div className="space-y-3 mb-4"><input value={colName} onChange={e=>setColName(e.target.value)} className="w-full px-4 py-2.5 rounded-xl border border-surface-200 focus:border-brand-400 outline-none text-sm" placeholder="集名称"/><textarea value={colDesc} onChange={e=>setColDesc(e.target.value)} className="w-full px-4 py-2.5 rounded-xl border border-surface-200 focus:border-brand-400 outline-none text-sm resize-none h-20" placeholder="描述（可选）"/></div>
          {chats.length>0&&(<div className="mb-4"><p className="text-xs font-medium text-surface-600 mb-2">选择对话</p><div className="max-h-40 overflow-y-auto space-y-1 border border-surface-200 rounded-xl p-2">{chats.map(c=>(<label key={c.id} className="flex items-center gap-2 p-2 rounded-lg hover:bg-surface-50 cursor-pointer text-sm"><input type="checkbox" checked={colChats.includes(c.id)} onChange={e=>{if(e.target.checked)setColChats([...colChats,c.id]);else setColChats(colChats.filter(x=>x!==c.id));}} className="accent-brand-500"/><span className="truncate">{c.title}</span></label>))}</div></div>)}
          <div className="flex gap-3"><button onClick={()=>setColModal(false)} className="flex-1 py-2.5 rounded-xl border border-surface-200 text-sm font-semibold text-surface-600">取消</button><button onClick={createCol} disabled={saving||!colName.trim()} className="flex-1 py-2.5 rounded-xl bg-brand-500 text-white text-sm font-semibold hover:bg-brand-600 disabled:opacity-50">{saving?'创建中...':'创建'}</button></div>
        </div>
      </Modal>
    </div>
  );
